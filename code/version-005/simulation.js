  ...
  State.nodes.forEach((node) => {
    if (node.ctrl === "signalized" && node.lights && node.lights.length > 0) {
      ...
      if (node.ai.timeInPhase >= currentGreenDuration) {
        ...
        // --- NEW SPEED-WEIGHTED ALGORITHM ---
        let totalWeightedVolume = 0;

        // 1. Calculate the total size of the pie, factoring in speed limits
        node.lights.forEach((id) => {
          const edge = getEdge(id);
          const speedWeight = edge ? edge.spd / 50 : 1.0; // Baseline is 50 km/h
          totalWeightedVolume +=
            (node.ai.historicalVolume[id] || 1) * speedWeight;
        });

        // 2. Distribute the green time based on the weighted proportions
        node.lights.forEach((id) => {
          const edge = getEdge(id);
          ...
          const proportion = weightedVolume / totalWeightedVolume;
