...
function showMoEdge(id) {
  ...
  mb.innerHTML = `
        ...
        <div class="mf"><label>SPEED LIMIT</label><input type="number" id="me3" min="10" max="120" value="${edge.spd}"></div>
...
function mOkEditEdge() {
  ...
  // Safety: If lanes decrease, clamp any existing cars to the new max lane so they don't draw outside the road
  ...
  saveState(); // Saves dynamically to localStorage!
}
...
function frame(timestamp) {
  ...
  if (State.simulation.isRunning) {
    if (typeof updateTrafficLights === "function") updateTrafficLights(simDt);
    if (typeof updateCars === "function") updateCars(simDt);
