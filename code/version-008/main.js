...
function showMoNode(id) {
    ...
    inEdges.forEach((ie) => {
      outEdges.forEach((oe) => {
        ...
        turnsHTML += `<label style="display:flex; align-items:center; gap:8px; margin-bottom:6px; font-size:11px; cursor:pointer;">
                    ...
                    <span style="color:var(--color-text-primary)">From <b>${prevN.lbl}</b> to <b>${nextN.lbl}</b> <i style="color:var(--color-text-tertiary)">(${tType})</i>${widthTag}</span>
                </label>`;
      });
    });
    turnsHTML += `</div>`;
  ...
  // Portal status badge for dangling nodes
  ...
  if (
    ...
    typeof getPortalForEndpoint === "function"
  ) {
    const portal = getPortalForEndpoint(
      ActiveAreaId,
      node.id,
      connectedEdges[0].id,
    );
    if (portal) {
      const dest = getOtherEndpoint(portal, ActiveAreaId, node.id);
      const destArea = Areas.get(dest.areaId);
      const destNode = destArea?.nodes.find((n) => n.id === dest.nodeId);
      portalBadge = `<div style="margin-bottom:10px; padding:6px 10px; background:rgba(168,85,247,0.12); border:1px solid #a855f7; border-radius:4px; font-size:10px; color:#d8b4fe;">
        ⇄ PORTAL → <b>${destArea?.name || "?"}</b> · ${destNode?.lbl || "?"}
      </div>`;
    } else {
      portalBadge = `<div style="margin-bottom:10px; padding:6px 10px; background:rgba(168,85,247,0.06); border:1px dashed #a855f760; border-radius:4px; font-size:10px; color:#a855f7;">
        ⇄ Dangling node — can be linked as a portal in ⇄ PORTALS
      </div>`;
    }
  }
  ...
  document.getElementById("mb").innerHTML = `
        <h4>Configure Intersection (${node.lbl.replace(/</g, "&lt;").replace(/>/g, "&gt;")})</h4>
        ${portalBadge}
        <div class="mf"><label>LABEL</label><input type="text" id="mn1" value="${node.lbl.replace(/"/g, "&quot;")}"></div>
        <div class="mf"><label>CONTROL TYPE</label>
           <select id="mn2" onchange="_onCtrlTypeChange(this.value)">
             <option value="signalized" ${node.ctrl === "signalized" ? "selected" : ""}>Signalized (Traffic Lights)</option>
             <option value="uncontrolled" ${node.ctrl === "uncontrolled" ? "selected" : ""}>Uncontrolled (Give Way)</option>
           </select>
        </div>
        <div class="mf" id="mn-cycle-row" style="display:${isSignalized ? "" : "none"}"><label>CYCLE LENGTH (SEC)</label><input type="number" id="mn3" min="10" max="240" value="${node.cycle}"></div>
...
// M3: Named handler for the control-type <select> in the node config modal.
// Replaces the previous inline onchange string which broke if node.lbl
// contained quote characters.
function _onCtrlTypeChange(value) {
  const sig = value === "signalized";
  const cycleRow = document.getElementById("mn-cycle-row");
  const lightsRow = document.getElementById("mn-lights-row");
  if (cycleRow) cycleRow.style.display = sig ? "" : "none";
  if (lightsRow) lightsRow.style.display = sig ? "" : "none";
}
...
function toggleSim() {
  // Delegate to areas.js which starts/stops ALL area sims simultaneously
  toggleAllSims();
}
...
function mOk() {
  ...
  const len = parseInt(document.getElementById("mo4").value) || 100;
...
function mOkEditEdge() {
  ...
  const newLen = parseInt(document.getElementById("me4").value) || 100;
...
function deleteNode(id) {
  // Remove any portals referencing this node before deleting
  if (typeof Portals !== "undefined") {
    Portals = Portals.filter(
      (p) =>
        !(p.a.areaId === ActiveAreaId && p.a.nodeId === id) &&
        !(p.b.areaId === ActiveAreaId && p.b.nodeId === id),
    );
    if (typeof savePortals === "function") savePortals();
...
function deleteEdge(id) {
  // Remove portals referencing this edge
  if (typeof Portals !== "undefined") {
    Portals = Portals.filter(
      (p) =>
        !(p.a.areaId === ActiveAreaId && p.a.edgeId === id) &&
        !(p.b.areaId === ActiveAreaId && p.b.edgeId === id),
    );
    if (typeof savePortals === "function") savePortals();
    if (typeof updatePortalBadges === "function") updatePortalBadges();
  }
...
// ─── Area Management UI Helpers ───────────────────────────────────────────────
function promptNewArea() {
  const name = prompt("Name for the new area:", `Area ${Areas.size + 1}`);
  if (!name) return;
  const id = createArea(name.trim());
  switchArea(id);
}

function promptRenameArea() {
  if (!ActiveAreaId) return;
  const area = Areas.get(ActiveAreaId);
  const name = prompt("Rename area:", area?.name || "");
  if (!name) return;
  renameArea(ActiveAreaId, name.trim());
  rebuildAreaDropdown();
}
...
function getHoveredEdge(wx, wy) {
  for (const e of State.edges) {
    ...
    const ax = wx - geom.A.x,
      ay = wy - geom.A.y;
    const t = (ax * geom.ux + ay * geom.uy) / geom.length;
...
// ─── MAIN ANIMATION LOOP ──────────────────────────────────────────────────────
let _lastDropdownRefresh = 0;

function frame(timestamp) {
  requestAnimationFrame(frame);
  resizeCanvas();

  // Tick all background area sims (areas not currently displayed)
  tickAllBackgroundAreas(timestamp);

  // Refresh dropdown car-count indicators ~once per second
  if (timestamp - _lastDropdownRefresh > 1000) {
    _lastDropdownRefresh = timestamp;
    rebuildAreaDropdown();
  }
  ...
  // Draw portal rings on top of intersections
  if (typeof drawPortalIndicators === "function") drawPortalIndicators();
  ...
  // M7: drawCars is intentionally called outside the isRunning block so that
  // cars remain visible on screen after the simulation is paused/stopped.
  // State.cars is cleared by toggleAllSims when stopped, so this is a no-op
  // in the stopped state (no cars to draw) and costs nothing at 60 fps.
