...
function spawnCars() {
  ...
  // M1: The original expression (Math.random() > POISSON_MEAN * density) has
  // a product that can exceed 1.0 at high densities, making the condition
  // permanently false and spawning a car on every attempt.  Normalise against
  // MAX_DENSITY so the probability is always in [0, 1].
  ...
  // S4: Instead of popping from the tail (which may remove cars mid-journey
  // without crediting throughput), find the car closest to the start of its
  // edge (lowest progress) and remove it.  This minimises disruption to
  // in-progress journeys when the density slider is reduced.
  while (State.cars.length > targetCount) {
    let minProgress = Infinity;
    let minIdx = State.cars.length - 1;
    for (let k = 0; k < State.cars.length; k++) {
      if (State.cars[k].progress < minProgress) {
        minProgress = State.cars[k].progress;
        minIdx = k;
      }
    }
    State.cars.splice(minIdx, 1);
  }
}
  ...
  State.nodes.forEach((node) => {
    ...
    if (!node.ai || !node.ai.phaseDurations) {
      ...
      node.ai = {
        ...
        memory: {},
        allRedTimer: 0,
        // C4: Always start in all-red so every intersection goes through a
        // clearance interval before any green is issued.  This matches real
        // controller cold-start behaviour and prevents conflicting greens when
        // two adjacent nodes are initialised in the same tick.
        inAllRed: true,
        preempting: false,
        preemptTimer: 0,
        watchdogAccum: 0,
    ...
    // C2: Run the watchdog BEFORE advancing timers so it evaluates the
    // accumulated time from previous ticks, not the value inflated by this
    // tick's dt.  This keeps the check logically consistent and prevents a
    // spurious all-red on the tick immediately following a normal transition.
    watchdogCheck(node);
    ...
    if (node.ai.timeInPhase >= currentGreenDuration) {
      ...
      node.lights.forEach((id) => {
        ...
        if (edge) {
          // BUG FIX: use logical length or fallback to pixel length — never undefined
      ...
      node.lights.forEach((id) => {
        ...
        // C3: Guard the available green budget.  If ALL_RED_SEC * phases
        // exceeds node.cycle (possible with many lights and/or a short cycle),
        // the raw budget would be negative, skewing every proportion and
        // forcing all phases to MIN_GREEN_SEC while the effective cycle length
        // silently balloons.  Clamp to at least one MIN_GREEN_SEC per phase.
        const rawBudget =
...
function updateCars(dt) {
  ...
  for (let i = State.cars.length - 1; i >= 0; i--) {
    ...
    if (car.progress >= 1) {
      ...
      // ── PORTAL CHECK ──────────────────────────────────────────────────────────
      // If the target node is dangling (only one connected edge) and has a portal,
      // teleport this car into the partner area instead of routing normally.
      const connectedToTarget = State.edges.filter(
        (e) => e.from === targetNodeId || e.to === targetNodeId,
      );
      if (
        connectedToTarget.length === 1 &&
        typeof schedulePortalTransfer === "function"
      ) {
        const transferred = schedulePortalTransfer(car, targetNodeId, edge);
        if (transferred) {
  ...
  // Flush any portal transfers scheduled this tick
  if (typeof flushPortalTransfers === "function") flushPortalTransfers();
}
