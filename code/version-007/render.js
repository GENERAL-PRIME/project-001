...
function drawTrafficLights() {
  State.nodes.forEach((node) => {
    ...
    node.lights.forEach((edgeId) => {
      ...
      // FIX (Missing): All-red is a distinct state — render amber/off colour
      let color;
      if (node.ai?.inAllRed) {
        color = "#ff9900"; // Amber during clearance
      } else {
...
function drawCars() {
  State.cars.forEach((car) => {
    ...
    // FIX (Missing): Emergency cars rendered in magenta with larger body
